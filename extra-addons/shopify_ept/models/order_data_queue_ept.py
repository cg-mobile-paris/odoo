...
class ShopifyOrderDataQueueEpt(models.Model):
        ...
        if order_ids:
            ...
            if results:
                ...
                if instance.create_shopify_orders_webhook:
                    order_queues = self.create_webhook_order_queue_ept(results,instance,queue_type, created_by="import")
                else:
    ...
    def prepare_existing_shopify_order_query_ept(self, results, instance):
        ...
        query = """select shopify_order_id from sale_order where shopify_order_id %s and shopify_instance_id = %s"""%(domain, instance.id)
