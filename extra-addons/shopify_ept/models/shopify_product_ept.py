...
class ShopifyProductProductEpt(models.Model):
    ...
    def shopify_export_products(self, instance, is_set_basic_detail, is_set_price, is_set_images, is_publish,
        ...
        for template in templates:
            ...
            if not result:
                ...
                common_log_line_obj.create_common_log_line_ept(shopify_instance_id=instance.id, message=message,module="shopify_ept",
                                                               model_name=model)
            if result:
    ...
    def request_for_shopify_template(self, template, model, instance):
        ...
        try:
            ...
            self.env["common.log.lines.ept"].create_common_log_line_ept(shopify_instance_id=instance.id,module="shopify_ept",
                                                                        message=message,
                                                                        model_name=model)
            return False
        return new_product
    ...
    def shopify_create_log(self, instance, message=False, model=False):
        ...
        @Task ID: 157623
        """
        log_line = self.env["common.log.lines.ept"].create_common_log_line_ept(shopify_instance_id=instance.id,module="shopify_ept",
                                                                               message=message,
                                                                               model_name=model)
        return log_line
