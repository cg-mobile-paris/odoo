...
class MSLogin(OAuthLogin):
    ...
    def signin(self, **kw):
        ...
        if not request.params.get('code'):
            return request.render('http_routing.http_error', {
                'status_code': _('Bad Request'),
                'status_message': _('Expected "code" param in URL, but its not there. Try again. Or contact Administrator')
            })
    ...
    def _ensure_password(self, login):
        ...
        if not odoo_user:
            ...
            if param:
                ...
                elif param.value.isnumeric():
                    template_user = request.env['res.users'].sudo().search([('id', '=', int(param.value))])
                    if not template_user:
                        return False
                    odoo_user = template_user.copy({
                        'login': login,
                        'name': login,
                        'image_1920': False
                    })
